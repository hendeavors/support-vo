...
class EmailValidationTest extends \Orchestra\Testbench\TestCase
{
    ...
    public function testLooseValidEmailsDoesNotThrowException()
    {
        foreach($this->validEmails as $validEmail) {
            EmailAddress::loose($validEmail);
        }
    }
     ...
     */
    public function testStrictValidEmailsThrowsException()
    {
        foreach($this->validEmails as $validEmail) {
            EmailAddress::strict($validEmail);
        }
    }
    
    /**
     * Increment a counter to ensure the size of the list of invalid emails
     * Is equivalent to the counter after all emails have been checked
     */
    public function testLooseInvalidEmailsDoesThrowException()
    {
        $i = 0;
        ...
        $this->assertEquals($i, count($this->invalidEmails));
    }
}